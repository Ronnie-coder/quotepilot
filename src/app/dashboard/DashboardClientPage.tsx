...
type DashboardClientPageProps = {
  ...
  quoteCount: number;
  totalRevenue: number;
  outstandingRevenue: number; // 🟢 New Prop
  recentDocuments: any[];
    ...
    <Box>
      <Flex justify="space-between" align="center" mb={4}>
        <HStack>
          ...
          <Heading as="h2" size="lg" color={headingColor} letterSpacing="tight">Action Required</Heading>
        </HStack>
      </Flex>
      <Alert
        status="warning" variant="subtle" flexDirection="column" alignItems="stretch"
        p={0} borderRadius="xl" borderWidth="1px" borderColor={borderColor}
        bg={bg} shadow="sm" overflow="hidden"
      >
        <Box p={6}>
          <HStack spacing={3}>
        ...
        <VStack spacing={0} align="stretch" borderTopWidth="1px" borderColor={borderColor}>
          {displayedInvoices.map((invoice, index) => (
            <ChakraLink
              as={NextLink} href={`/quote/${invoice.id}`} key={invoice.id}
              _hover={{ textDecoration: 'none', bg: useColorModeValue('orange.100', 'whiteAlpha.200') }}
              p={4} px={6} w="full"
              borderBottomWidth={index === displayedInvoices.length - 1 && !hasHiddenItems ? '0px' : '1px'}
              borderColor={borderColor} transition="background-color 0.2s"
            >
              <Flex w="full" justify="space-between" align="center">
                <VStack align="start" spacing={0}>
                  <Text fontWeight="bold" color={headingColor} fontSize="sm">#{invoice.invoice_number || 'N/A'} — {invoice?.clients?.name}</Text>
                  <HStack spacing={1} mt={1}>
                     <Icon as={Clock} boxSize={3} color={warningColor} />
                     <Text color={warningColor} fontSize="xs" fontWeight="bold">Due: {new Date(invoice.due_date).toLocaleDateString()}</Text>
                  </HStack>
                </VStack>
        ...
        {hasHiddenItems && (
          <Button
            onClick={() => setShowAll(!showAll)} variant="ghost" colorScheme="orange" size="sm" width="full" rounded="none" py={6}
            rightIcon={<Icon as={showAll ? ChevronUp : ChevronDown} />}
            _hover={{ bg: useColorModeValue('orange.100', 'whiteAlpha.200') }}
          >
            {showAll ? 'Show Less' : `View ${hiddenCount} More Overdue Invoices`}
          </Button>
        )}
      </Alert>
    </Box>
...
export default function DashboardClientPage({
  ...
  return (
    <>
      <VStack as={motion.div} variants={containerVariants} initial="hidden" animate="visible" spacing={8} align="stretch" pb={12}>
        
        {/* 1. Header */}
        <Flex as={motion.div} variants={itemVariants} direction={{ base: 'column', md: 'row' }} justify="space-between" align={{ base: 'start', md: 'center' }} gap={4}>
          <Box>
            ...
            <HStack mt={1}>
              ...
              <Text color={textColor} fontSize="sm">System Operational</Text>
            </HStack>
          </Box>
          <Menu>
        ...
        <SimpleGrid as={motion.div} variants={itemVariants} columns={{ base: 1, md: 2, lg: 4 }} spacing={6}>
          ...
          {/* 🟢 OUTSTANDING (Orange) - Replaces "Active" */}
        ...
        <SimpleGrid columns={{ base: 1, lg: 3 }} spacing={8} as={motion.div} variants={itemVariants}>
          {/* Revenue Trend Chart */}
          <Box gridColumn={{ lg: 'span 2' }} bg={cardBg} p={6} borderRadius="xl" borderWidth="1px" borderColor={borderColor} shadow="sm" position="relative">
          ...
          <Flex direction="column" bg={cardBg} borderRadius="xl" borderWidth="1px" borderColor={borderColor} shadow="sm" p={6} h="full" minH="400px">
            ...
            <Box flex="1" h="250px" position="relative">
              {totalInvoices > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
