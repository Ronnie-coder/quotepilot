...
export default function DashboardClientPage({
  ...
  // 🟢 COMMANDER FIX: Explicit tooltip colors for Dark Mode clarity
  const tooltipBg = useColorModeValue('#ffffff', '#2D3748'); // White in light, Dark Grey in dark
  const tooltipBorder = useColorModeValue('#E2E8F0', '#4A5568');
  const tooltipText = useColorModeValue('#1A202C', '#F7FAFC');
  
  const [brand500, green400, orange400, gray300, blue400] = useToken('colors', [
    'brand.500', 
    'green.400', 
    'orange.400', 
    'gray.300',
  ...
  const overdueCount = statusData.find(s => s.name === 'Overdue')?.value || 0;
    ...
    <>
      ...
      >
        ...
        >
          <Box>
            <Heading as="h1" size="lg" color={headingColor} letterSpacing="tight">
          ...
          <Menu>
            ...
            <MenuList shadow="xl" borderColor={borderColor}>
              <MenuItem as={NextLink} href="/quote/new" icon={<Icon as={Plus} boxSize={4} />}>
                New Document
              </MenuItem>
              <MenuItem onClick={onOpen} icon={<Icon as={UserPlus} boxSize={4} />}>
                Add New Client
              </MenuItem>
            </MenuList>
          </Menu>
        ...
        <SimpleGrid columns={{ base: 1, lg: 3 }} spacing={8} as={motion.div} variants={itemVariants}>
             ...
             <Box h="300px" w="99%">
               {revenueData && revenueData.length > 0 ? (
                 <ResponsiveContainer width="100%" height="100%">
                   <AreaChart data={revenueData}>
                     ...
                     <XAxis 
                        ...
                        axisLine={false} 
                        tickLine={false} 
                        tick={{ fill: textColor === 'gray.400' ? '#A0AEC0' : '#718096', fontSize: 12 }} 
                        dy={10}
                     />
                     <YAxis hide />
                     <Tooltip 
                        // 🟢 COMMANDER FIX: Applying explicit tooltip styles
                        contentStyle={{ 
                 ...
                 <Flex h="100%" align="center" justify="center" direction="column" color="gray.400">
                    <Text fontSize="sm">Not enough data to display chart.</Text>
                 </Flex>
               )}
             </Box>
          </Box>

          {/* RIGHT: Invoice Status Distribution (33% Width) */}
          <Flex
             direction="column"
             bg={cardBg}
             borderRadius="xl"
          ...
          >
            <HStack mb={4}>
              <Icon as={PieChartIcon} color="orange.400" boxSize={5} />
              <Heading size="md" color={headingColor}>Invoice Health</Heading>
            </HStack>
            ...
            <Box flex="1" h="250px" position="relative">
              {totalInvoices > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                 ...
                 <Flex h="100%" align="center" justify="center" direction="column" color="gray.400">
                    <Text fontSize="sm">No invoice data available.</Text>
                 </Flex>
              )}
            </Box>
            ...
            <HStack justify="space-around" mt={4} pt={4} borderTopWidth="1px" borderColor={borderColor}>
              <VStack spacing={0}>
                ...
                <Text fontWeight="bold" color="green.500">
                  {totalInvoices ? Math.round((paidCount / totalInvoices) * 100) : 0}%
                </Text>
              </VStack>
              <VStack spacing={0}>
                <Text fontSize="xs" color="gray.500">Overdue</Text>
                <Text fontWeight="bold" color="orange.500">
                  {totalInvoices ? Math.round((overdueCount / totalInvoices) * 100) : 0}%
                </Text>
              </VStack>
            </HStack>
          </Flex>
            ...
            <VStack spacing={0} align="stretch">
              {recentDocuments && recentDocuments.length > 0 ? (
                    ...
                    <HStack justify="space-between">
                      <HStack spacing={3}>
                        <Box
                          p={2}
                          borderRadius="md"
                          bg={doc.document_type === 'Invoice' ? 'green.100' : 'purple.100'}
                          color={doc.document_type === 'Invoice' ? 'green.600' : 'purple.600'}
                        >
                      ...
                      <Text fontWeight="bold" fontSize="sm" color={headingColor} fontFamily="mono">
                ...
                <Flex direction="column" align="center" justify="center" p={10}>
                  <Icon as={Inbox} boxSize={10} color={textColor} opacity={0.5} mb={3} />
                  <Text color={textColor} fontSize="sm">No recent activity.</Text>
                </Flex>
              )}
            </VStack>
        </Box>
